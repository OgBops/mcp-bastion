...
Override via environment variables for unusual workloads, e.g.:
    MCP_BASTION_MAX_FRAME_BYTES=4194304
"""
...
MAX_FRAME_BYTES: int = _env_int("MCP_BASTION_MAX_FRAME_BYTES", 1 * 1024 * 1024)

# An entire HTTP POST body (one JSON-RPC message or a small batch).
MAX_HTTP_BODY_BYTES: int = _env_int("MCP_BASTION_MAX_HTTP_BODY_BYTES", 4 * 1024 * 1024)

# JSON-RPC nesting depth. JSON-RPC structure is shallow; legitimate tool
# arguments rarely exceed 10 levels.
MAX_JSON_DEPTH: int = _env_int("MCP_BASTION_MAX_JSON_DEPTH", 64)
...
REGEX_TIMEOUT_SECONDS: float = float(
    os.environ.get("MCP_BASTION_REGEX_TIMEOUT_SECONDS", "0.5")
)
...
MAX_REGEX_PATTERN_LEN: int = _env_int("MCP_BASTION_MAX_REGEX_PATTERN_LEN", 1024)
...
REDACT_WINDOW_BYTES: int = _env_int("MCP_BASTION_REDACT_WINDOW_BYTES", 64 * 1024)

# Bounded recursion is a worse trade-off than bounded *work*. v0.3.1 walks
# the JSON tree iteratively with a node-count cap.
MAX_REDACT_DEPTH: int = _env_int("MCP_BASTION_MAX_REDACT_DEPTH", 4096)
...
# tree shape.
MAX_REDACT_NODES: int = _env_int("MCP_BASTION_MAX_REDACT_NODES", 1_000_000)
