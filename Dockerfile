# mcp-firewall — image suitable for both regular Docker and AWS Nitro Enclaves.
#
# Build (regular):
...
#   docker build -t mcp-firewall:0.2.0 .
#   nitro-cli build-enclave --docker-uri mcp-firewall:0.2.0 \
#     --output-file mcp-firewall.eif
#   nitro-cli run-enclave --cpu-count 2 --memory 2048 \
#     --eif-path mcp-firewall.eif --enclave-cid 16
#
...
COPY examples/policy.yaml /etc/mcp-firewall/policy.yaml

EXPOSE 8080
ENTRYPOINT ["mcp-firewall"]
CMD ["up", "--policy", "/etc/mcp-firewall/policy.yaml", \
     "--listen", "0.0.0.0:8080", \
