# Wiring mcp-bastion into Claude Desktop

This guide turns mcp-bastion into the **default** wrapper for all your MCP
...
- Python 3.11+ on `PATH`
- mcp-bastion installed (`pip install -e ".[dev]"` from this repo)

## Step 1 — generate a policy

```bash
mkdir -p ~/.mcp-bastion
mcp-bastion init -o ~/.mcp-bastion/policy.yaml
```

Edit `~/.mcp-bastion/policy.yaml` to taste. The starter policy denies
  ...
  "mcpServers": {
    "filesystem": {
      ...
      "args": [
        "up",
        "--policy",
        "/Users/you/.mcp-bastion/policy.yaml",
        "--upstream",
...
```json
{
  "mcpServers": {
    "filesystem": {
      "command": "mcp-bastion",
      "args": [
        "up", "--policy", "/Users/you/.mcp-bastion/policy.yaml",
...
```bash
mcp-bastion inspect-log --policy ~/.mcp-bastion/policy.yaml --verify --limit 20
```
...
**Claude Desktop can't find `mcp-bastion`** — Claude Desktop spawns child
processes with a minimal `PATH`. Use the absolute path of the binary in the
`command` field:

```bash
which mcp-bastion   # e.g. /Users/you/.venv/bin/mcp-bastion
```

Replace `"command": "mcp-bastion"` with `"command": "/Users/you/.venv/bin/mcp-bastion"`.

**Tool calls fail with `denied by mcp-bastion`** — that's working as
...
Use the same `mcp-bastion wrap` command to generate the snippet for any client.
